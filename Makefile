...
EXAMPLE_FLAGS = -I./examples -O0 -g3 -std=c++11 -fportcosmo
EXAMPLE_MODFLAGS = $(EXAMPLE_FLAGS) -DUSING_PLUGIN=1\
				   -include examples/tmpconst.h -include ./tmpconst.h
EXAMPLE_RESFLAGS = $(EXAMPLE_FLAGS)\
				   -include examples/tmpconst.h -include ./tmpconst.h

EXAMPLE_SOURCES = $(wildcard examples/ex*.cc)
EXAMPLE_RUNS = $(EXAMPLE_SOURCES:%.cc=%.runs)
EXAMPLE_MODBINS = $(EXAMPLE_SOURCES:examples/%.cc=examples/modded_%)
EXAMPLE_RESBINS = $(EXAMPLE_SOURCES:examples/%.cc=examples/result_%)
...
./examples/%.o: ./examples/%.cc ./examples/tmpconst.h
	$(CXX) $(EXAMPLE_FLAGS) $< -c -o $@

./examples/modded_%.o: ./examples/%.cc
	$(CXX) $(EXAMPLE_MODFLAGS) $< -c -o $@

./examples/modded_%: ./examples/functions.o ./examples/supp.o ./examples/modded_%.o 
	$(PLUGIN_CXX) -o $@ $^
